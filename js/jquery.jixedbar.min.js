...
(function() {
    ...
    // console.log("en head"+browser);
    })();

(function ($) {
    // console.log("en jixed");
    // console.log($.browser);
  $.fn.jixedbar = function (options) {
    ...
    this.each(function () {
      ...
      if ($(".jx-bar-button-right", this).exists()) {
        $("<ul />")
          .attr("id", "jx-hid-con-id")
          .insertBefore($(this).find(".jx-bar-button-right:first"));
      } else {
      ...
      $("#jx-hid-con-id").html(
        '<li alt="Ocultar Barra"><a id="jx-hid-btn-id" class="' +
          hideIndicator +
          '"></a></li>'
      );
      ...
      $(this).css({ overflow: constants["constOverflow"], position: pos });
      if (defaults.showOnTop) {
        $(this).css({ top: constants["constBottom"] });
      } else {
        $(this).css({ bottom: constants["constBottom"] });
      }
      ...
      $("#jx-uhid-con-id").css({
        ...
        "margin-left":
          $(this).offset().left +
          $(this).width() -
          $("#jx-uhid-con-id").width(),
      });
      if (defaults.showOnTop) {
        $("#jx-uhid-con-id").css({ top: constants["constBottom"] });
      } else {
        $("#jx-uhid-con-id").css({ bottom: constants["constBottom"] });
      }
      if (defaults.transparent) {
      ...
      $("#jx-uhid-itm-id").html(
        '<li alt="Mostrar Barra"><a id="jx-uhid-btn-id" class="' +
          unhideIndicator +
          '"></a></li>'
      );
      if (defaults.roundedCorners) {
      ...
      $("li", obj).hover(
        ...
        //   console.log($(this).attr("alt"));
          ...
          if (tooltipTitle == "") {
            tooltipTitle = $(this).attr("id");
          }
          if (tooltipTitle != "") {
            barTooltipWrapperID = barTooltipID + "_wrapper";
            $("<div />")
              .attr("id", barTooltipWrapperID)
              .appendTo("#jx-ttip-con-id");
            $("<div />")
              .attr("id", barTooltipID)
              .appendTo("#" + barTooltipWrapperID);
            $("#" + barTooltipID).css({ float: "left" });
            if (defaults.showOnTop && !($.browser.msie && ie6)) {
              $("<div />")
                .addClass("jx-tool-point-dir-up")
                .appendTo("#" + barTooltipID);
            }
            $("<div />")
              .html(tooltipTitle)
              .addClass("jx-bar-button-tooltip")
              .appendTo("#" + barTooltipID);
            if (!defaults.showOnTop && !($.browser.msie && ie6)) {
              $("<div />")
                .addClass("jx-tool-point-dir-down")
                .appendTo("#" + barTooltipID);
            }
            ...
            $("#" + barTooltipWrapperID).css({
              "margin-left":
                $(this).offset().left -
                $("#" + barTooltipID).width() / 2 +
                $(this).width() / 2 +
                lft_pad,
            });
            if (
              $(this).find("a:first").attr("name") == "" ||
              button_active == false
            ) {
              $("#" + barTooltipID).fadeTo(
                defaults.tooltipFadeSpeed,
                defaults.tooltipFadeOpacity
              );
            } else if (
              active_button_name != $(this).find("a:first").attr("name")
            ) {
              $("#" + barTooltipID).fadeTo(
                defaults.tooltipFadeSpeed,
                defaults.tooltipFadeOpacity
            ...
            $(this)
              ...
              .each(function () {
                ...
                $(this)
                  .parent()
                  .html(
                    "<span style=\"cursor:pointer;display:inline-block;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" +
                      imgPath +
                      "');\">" +
                      srcText +
                      "</span> " +
      ...
      $("li", obj)
        ...
        .each(function () {
          ...
          if (($.browser.msie && ie6) || ($.browser.msie && ie7)) {
            $("<div />")
              .attr("class", buttonIndicator)
              .insertBefore($(this).parent().find("a"))
              .css({ "background-position": "top" });
          } else {
            $("<div />")
              .attr("class", buttonIndicator)
              .insertAfter($(this).parent().find("a"));
          }
          $(this)
            .parent()
            .find("a:first")
            .click(function () {
              ...
              if (button_active && active_button_name == $(this).attr("name")) {
                ...
                $("#jx-menu-con-id").html(
                  "<ul>" + $(this).parent().find("ul").html() + "</ul>"
                );
                $("#jx-menu-con-id").css({
                  overflow: constants["constOverflow"],
                  position: pos,
                ...
                if ($.browser.msie && ie6) {
                  $("#jx-menu-con-id ul li a").css({ width: "100%" });
                }
                if (defaults.roundedButtons) {
                ...
                if (active_button_name != "") {
                  $("a[name='" + active_button_name + "']")
                    .parent()
                    .removeClass("jx-nav-menu-active");
                  $("a[name='" + active_button_name + "']")
                    .parent()
                    .removeClass("jx-nav-menu-active-rounded");
                  if (defaults.showOnTop) {
                    buttonIndicator = "jx-arrow-down";
                  } else {
                    buttonIndicator = "jx-arrow-up";
...
jQuery.fn.createCookie = function (cookie_name, value) {
  ...
  document.cookie =
    cookie_name + "=" + escape(value) + ";expires=" + expiry_date.toUTCString();
};
